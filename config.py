...
def parse_args():
    ...
    parser.add_argument('--train_data_path_list',
                        default='/home/aizoo/data/workspace/GT-U-Net-master/prepare_dataset/data_path_list/STARE/train.txt')
    parser.add_argument('--test_data_path_list',
                        default='/home/aizoo/data/workspace/GT-U-Net-master/prepare_dataset/data_path_list/STARE/test.txt')
    ...
    parser.add_argument('--stride_height', default=440)
    parser.add_argument('--stride_width', default=440)
    ...
    # === STARE数据集特殊优化参数 === #
    parser.add_argument('--stare_lr', default=0.0001, type=float,  # 降低学习率
                       help='Lower learning rate for STARE dataset')
    parser.add_argument('--stare_batch_size', default=2, type=int,  # 增加批次大小
                       help='Batch size for STARE dataset')
    parser.add_argument('--stare_epochs', default=250, type=int,    # 增加训练轮数
                       help='More epochs for STARE dataset')
    parser.add_argument('--stare_warmup_epochs', default=30, type=int,
                       help='Warmup epochs for STARE')
    parser.add_argument('--stare_data_aug_strength', default=0.8, type=float,
                       help='Data augmentation strength for STARE')
    
    args = parser.parse_args()
    
    # === 针对STARE数据集的特殊调整 === #
    if 'STARE' in args.train_data_path_list:
        args.dataset_name = 'STARE'
        args.lr = args.stare_lr
        args.batch_size = args.stare_batch_size  
        args.N_epochs = args.stare_epochs
        args.N_patches = 2000  # 增加训练样本数
        print(f"检测到STARE数据集，应用特殊优化: lr={args.lr}, batch_size={args.batch_size}, epochs={args.N_epochs}")
    
    elif 'CHASEDB1' in args.train_data_path_list:
