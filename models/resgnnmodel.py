...
class ResBlockWithPool(nn.Module):
    def __init__(self, in_channels, out_channels):
        super(ResBlockWithPool, self).__init__()
...
class GNN_UNet(nn.Module):
    def __init__(self, img_ch=3, output_ch=1,):
        ...
        self.Conv2 = ResBlockWithPool(64, 128)
        self.Conv3 = ResBlockWithPool(128, 256)
        self.Conv4 = ResBlockWithPool(256, 512)
        self.Conv5 = ResBlockWithPool(512, 1024)
        ...
        self.gnn = GAT3D(in_channels=1024, out_channels=1024, num_layers=1, heads=2)  # heads不宜过大
    ...
    def forward(self,x):
        ...
        x2 = self.Conv2(x1)              # 1*64*256*256 ->conv(64,128)->conv(128,128)-> 1*128*256*256


        #x3 = self.Maxpool(x2)            # 1*128*256*256 -> 1*128*128*128
        x3 = self.Conv3(x2)              ## 1*128*128*128 ->conv(128,256)->conv(256,256)->  1*256*128*128
        

        
        #x4 = self.Maxpool(x3)            # 1*256*128*128 -> 1*256*64*64
        x4 = self.Conv4(x3)              ## 1*256*64*64 ->conv(256,512)->conv(512,512)-> 1*512*64*64

        
        
        
        #x5 = self.Maxpool(x4)            ## 1*512*64*64 -> 1*512*32*32
        x5 = self.Conv5(x4)             ## 1*512*32*32->conv(512,1024)->conv(1024,1024)-> 1*1024*32*32

        B, C, H, W = x5.shape
        x5_reshaped = x5.view(B, C, -1).permute(0, 2, 1)  # [B, N, C]
        x5_gnn = self.gnn(x5_reshaped)  # [B, N, C]
        x5_gnn = x5_gnn.permute(0, 2, 1).view(B, C, H, W)
        x5 = self.bn_gnn(x5_gnn + x5)  # 残差+归一化
