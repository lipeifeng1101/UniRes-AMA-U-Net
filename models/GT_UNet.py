...
class GT_U_Net(nn.Module):
    ...
    def forward(self, x):
        # encoding path
        x1 = self.Conv1(x) # 会变成64，64，64，64
        #输出的x1变为64 64 64 64
        x2 = self.Maxpool(x1) # 64 64 32 32
        x2 = self.Conv2(x2) # 64 128 32 32

        x3 = self.Maxpool(x2) #64 128 16 16
        x3 = self.Conv3(x3) # 64 256 16 16

        x4 = self.Maxpool(x3) # 64 256 8 8
        x4 = self.Conv4(x4) # 64 512 8 8

        x5 = self.Maxpool(x4) #64 512 4 4
        x5 = self.Conv5(x5) # 64 1024 4 4

        # decoding + concat path
        d5 = self.Up5(x5) # 64 512 8 8
        d5 = torch.cat((x4, d5), dim=1) # 64 1024 8 8
        d5 = self.Up_conv5(d5) # 64 512 8 8

        d4 = self.Up4(d5) # 64 256 16 16
        d4 = torch.cat((x3, d4), dim=1)# 64 512 16 16
        d4 = self.Up_conv4(d4) # 64 256 16 16

        d3 = self.Up3(d4) # {64,128,32,32}
        d3 = torch.cat((x2, d3), dim=1) #{64,256,32,32}
        d3 = self.Up_conv3(d3) #{64,128,32,32}

        d2 = self.Up2(d3) #{64,64,64,64}
        d2 = torch.cat((x1, d2), dim=1) #{64,128,64,64}
        d2 = self.Up_conv2(d2) #64 64 64 64

        d1 = self.Conv_1x1(d2)

        return d1 # 64 2 64 64
